...
git log --oneline | Журнал действий компактный вид по строкам
...
git reset \<commit hash\> | Отмена коммита
git revert \<commit hash\> | Не отменяет коммит, а создает новый коммит, который отменяет действия, совершенные в предыдущем
git reflog | Журнал всех операций, которые были совершены во время работы с репозиторием


что-то для теста команды git revert